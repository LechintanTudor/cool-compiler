...
pub fn resolve_fn(
    ...
    let ty_id = match explicit_ty {
        Some(ty_id) => {
            ...
            if let Some(expr_abi) = resolve_fn_expr_abi(fn_expr.abi_decl.as_ref())? {
            ...
            if fn_ty.param_tys.len() != fn_expr.params.params.len() {
                Err(ResolveError::FnParamCountMismatch {
                    found: fn_ty.param_tys.len() as _,
                    expected: fn_expr.params.params.len() as _,
                })?
            }
            ...
            let params_to_check = fn_ty
                .param_tys
                .iter()
                .zip(fn_expr.params.params.iter())
            ...
            if fn_ty.is_variadic != fn_expr.params.is_variadic {
                Err(ResolveError::FnVariadicMismatch {
                    found: fn_expr.params.is_variadic,
                })?
            }

            if let Some(expr_return_ty) = fn_expr.return_ty.as_ref() {
        ...
        None => {
            ...
            context.add_fn_ty(abi, param_tys, fn_expr.params.is_variadic, return_ty)
        }
    };
